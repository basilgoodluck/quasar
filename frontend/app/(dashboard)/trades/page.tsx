...
// Snap trade timestamp to nearest candle open time
...
export default function TradesPage() {
  ...
  // Only trades for the selected symbol
  ...
  const buildMarkers = useCallback((tradeList: Trade[]): SeriesMarker<Time>[] => {
    ...
    return tradeList
      ...
      .map(t => {
        ...
        // If snapped time not in candle set, skip marker — trade outside chart range
  ...
  const fetchHistoricalData = useCallback(async (symbol: string) => {
    try {
      ...
      // Store valid candle times for marker snapping
